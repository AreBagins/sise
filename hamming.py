...
def solve_puzzle_a_star(initial_state, goal_state, width, height, move_order):
    ...
    open_list = []
    heapq.heappush(open_list, (hamming_distance(initial_state, goal_state), 0, initial_state, []))  # (f(n), g(n), stan, ścieżka)
    visited = set()
...
# Uruchomienie algorytmu A* z metryką Hamming
